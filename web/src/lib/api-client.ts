...
async function response(path: string, init?: RequestInit) {
  ...
  catch { throw new ApiClientError('network_error', 'De server is niet bereikbaar. Controleer de verbinding en probeer opnieuw.'); }
  if (!result.ok) {
...
async function request<T>(path: string, init?: RequestInit): Promise<T> { return (await response(path, init)).json(); }
...
export async function askQuestion(question: string, sourceIds?: string[]): Promise<Answer> {
  if (!useFixtures) return request('/api/answers', json('POST', { question, ...(sourceIds ? { sourceIds } : {}) }));
...
export interface SettingsPatch {
  ...
  custom?: { baseUrl: string }; tts?: Partial<Pick<Settings['tts'], 'provider' | 'voiceId'>> & { key?: string | null };
...
export async function streamAnswer(question: string, handlers: { partial: (text: string) => void; final: (answer: Answer) => void }, sourceIds?: string[], signal?: AbortSignal): Promise<void> {
  ...
  const frame = (value: string) => {
    ...
    if (event === 'partial') handlers.partial(parsed.antwoord);
    if (event === 'final') { finished = true; handlers.final(parsed as Answer); }
