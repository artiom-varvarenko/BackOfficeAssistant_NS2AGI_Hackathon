...
export async function summarizeSource(sourceId: string, expectedVersionId?: string, language: Locale = 'nl'): Promise<Source> {
  const db = getDb();
  ...
  const result = await generateTextPlain('summary', { system: language === 'en' ? SYSTEM.replace('drie heldere Nederlandse zinnen', 'three clear English sentences') + '\nWrite all three sentences in English.' : SYSTEM, prompt: snapshot.prompt, maxOutputTokens: 600 });
  ...
  return db.transaction(() => {
    ...
    clearEnrichmentWarning(snapshot.versionId, 'summary', db);
